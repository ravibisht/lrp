  ...
  app.post("/api/audit-cjp-behavior", async (req, res) => {
    ...
    if (!client) {
      ...
      return res.json({
        success: true,
        isFallback: true,
        roast: `${name ? `Dear ${name} (Volunteer ID: LRP-MOCK-${Math.floor(1e3 + Math.random() * 9e3)})` : "Fellow citizen"}, based on our physical local ledger, here is your audit verdict: ${LOCAL_FALLBACK_ROASTS[randomIndex]}`,
